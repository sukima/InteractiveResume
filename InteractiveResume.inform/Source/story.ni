...
the InstantChat	"An end-to-end secure chat app designed to allow a Deaf person to send ephemeral messages to others by the use of a QR Code or small link. It uses LibSodium to encrypt the communications between the clients and the keys are only on the client browser till the tab closes. Everything is client side written in EmberJS with the server being only a Socket.IO relay."	"https://tritarget.org/#Instant%20Chat"	"EmberJS, JavaScript, CSS, LibSodium, Socket.io, Node.JS"
the TimerApp	"A simple yet useful speech or talk timer that follows the Toastmasters recommended timer notifications."	"https://tritarget.org/#Timer%20App"	"EmberJS, JavaScript, CSS"
the ConfirmedModule	"A framework agnostic asynchronous confirmation module for both Vanilla JS and Ember"	"https://tritarget.org/#Confirmed%20NPM%20Module:%5B%5BConfirmed%20NPM%20Module%5D%5D%20%5B%5BConfirmed%20Ember%20Addon%5D%5D"	"JavaScript, EmberJS, Promises"
...
A melted Klondike bar is a candy. It is contained in InstantChat. The description is "The melted vanilla ice cream surrounded by a cracked and melted chocolate shell is barely contained by the soggy foil wrapper.[if the player carries the melted klondike bar] Your hands get sticky as you hold it.[end if]".
